...
class StoreViewSet(viewsets.ViewSet, generics.ListAPIView):
    ...
    def get_serializer_class(self):
        if self.request.user.is_authenticated:
            return serializers.AuthenticatedFollowStoreDetailSerializer
    ...
    def follow(self, request, pk):
        fl, created = Follow.objects.get_or_create(store=self.get_object(),
                                                  user=request.user)
        if not created:
            fl.active = not fl.active
            fl.save()

        return Response(serializers.AuthenticatedFollowStoreDetailSerializer(self.get_object()).data)
...
class MenuItemViewSet(viewsets.ViewSet, generics.RetrieveAPIView):
    ...
    def like(self, request, pk):
        ...
        return Response(serializers.AuthenticatedMenuItemDetailSerializer(self.get_object()).data)
